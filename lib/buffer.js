window.Buffer = (function(){
   ...
   *  			When capacity is reached, the oldest element gets replaced
	 ...
	 *
   *  	2.) Regrow - will simply keep adding elements to the array
   *				much like a regular JS array.push()
	...
	var Buffer = function(options){
		 ...
		 */
		function makeSpace() {
			if (GROW_MODE === Buffer.GROW_MODE.REGROW)
				regrow();
			else
				incrementHead();
		};
		...
		/**
		 * Add a new element to the buffer (Enqueue)
		 *
		 * @param {this.DATA_TYPE} element
		 * @api public
		 */
		this.add = function(element){
			if (typeDoesNotMatch(element))
				throw new Buffer.InvalidTypeAddException();
		...
		/**
		 * Remove the oldest element from the buffer (Dequeue)
		 * 
	...
	/**
	 * Exception: Thrown if adding an element of incompatible type
	 */
	Buffer.InvalidTypeAddException = (function(){
		var error = function(){
			this.name = "InvalidTypeAddException";
			this.message = "Attempted to add an element to this buffer that did not match the buffer's DATA_TYPE.";
		};
		error.prototype = new Error();
		return error;
	})();
